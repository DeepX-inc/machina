...
from machina import distillation_loss as dl
...
def train(traj, student_pol, teacher_pol, student_optim, epoch, batchsize, num_epi_per_seq = 1):
    ...
    iterator = traj.iterate(batchsize, epoch) if not pol.rnn else traj.iterate_rnn(batchsize=batchsize, num_epi_per_seq = num_epi_per_seq, epoch = epoch)
