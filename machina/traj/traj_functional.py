...
def normalize_obs_and_acs(traj):
    with torch.no_grad():
        ...
        mean_next_obs = torch.mean(traj.data_map['next_obs'], dim=0, keepdim=True)
        std_next_obs = torch.std(traj.data_map['next_obs'], dim=0, keepdim=True)
        traj.data_map['next_obs'] = (traj.data_map['next_obs'] - mean_next_obs) / std_next_obs
